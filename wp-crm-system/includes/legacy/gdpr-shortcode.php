		...
		<table class="wpcrm_system_gdpr_data_table">
			...
			<tr id="contact_name">
				...
				<td class="wpcrm_system_gdpr_data">
					...
					if ( array_key_exists( 'first_name', $data ) && '' != $data['first_name'] ){
						echo '<span id="wpcrm_system_gdpr_table_first_name">' . esc_attr( $data['first_name'] ) . '</span> ';
					}
					if ( array_key_exists( 'last_name', $data ) && '' != $data['last_name'] ){
						echo '<span id="wpcrm_system_gdpr_table_last_name">' . esc_attr( $data['last_name'] ) . '</span>';
					}
					?>
				</td>
			</tr>
				...
				<td class="wpcrm_system_gdpr_data">
					...
					foreach ( $data['categories'] as $category ){
						if( is_object( $category ) ){
							...
							<span id="wpcrm_system_gdpr_table_category_<?php echo esc_attr( $cat_id ); ?>"><?php echo esc_attr( $category->name . $comma ); ?></span>
						<?php }
						$a++;
					} ?>
				</td>
			...
			<tr id="contact_name">
				...
				<td class="wpcrm_system_gdpr_data">
					...
					if ( array_key_exists( 'organization', $data ) && '' != $data['organization'] ){
						echo '<span id="wpcrm_system_gdpr_table_organization">' . esc_attr( $data['organization'] ) . '</span> ';
					}
					if ( array_key_exists( 'role', $data ) && '' != $data['role'] ){
						echo '<span id="wpcrm_system_gdpr_table_role">' . esc_attr( $data['role'] ) . '</span>';
					}
					?>
				</td>
			</tr>
			...
			<tr id="contact_address">
				...
				<td class="wpcrm_system_gdpr_data">
					...
					if ( array_key_exists( 'street_1', $data ) && '' != $data['street_1'] ){
						echo '<div id="wpcrm_system_gdpr_table_street_1">' . esc_attr( $data['street_1'] ) . '</div>';
					}
					if ( array_key_exists( 'street_2', $data ) && '' != $data['street_2'] ){
						echo '<div id="wpcrm_system_gdpr_table_street_2">' . esc_attr( $data['street_2'] ) . '</div>';
					}
					if ( array_key_exists( 'city', $data ) && '' != $data['city'] ){
						echo '<div id="wpcrm_system_gdpr_table_city">' . esc_attr( $data['city'] ) . '</div>';
					}
					if ( array_key_exists( 'state', $data ) && '' != $data['state'] ){
						echo '<div id="wpcrm_system_gdpr_table_state">' . esc_attr( $data['state'] ) . '</div>';
					}
					if ( array_key_exists( 'postal_code', $data ) && '' != $data['postal_code'] ){
						echo '<div id="wpcrm_system_gdpr_table_postal_code">' . esc_attr( $data['postal_code'] ) . '</div>';
					}
					if ( array_key_exists( 'country', $data ) && '' != $data['country'] ){
						echo '<div id="wpcrm_system_gdpr_table_country">' . esc_attr( $data['country'] ) . '</div>';
					}
					?>
				</td>
			</tr>
			...
			<tr id="contact_contact_information">
				...
				<td class="wpcrm_system_gdpr_data">
					...
					if ( array_key_exists( 'phone', $data ) && '' != $data['phone'] ){
						echo '<div id="wpcrm_system_gdpr_table_phone">' . esc_html__( 'Phone: ', 'wp-crm-system' ) . esc_attr( $data['phone'] ) . '</div>';
					}
					if ( array_key_exists( 'mobile', $data ) && '' != $data['mobile'] ){
						echo '<div id="wpcrm_system_gdpr_table_mobile">' . esc_html__( 'Mobile: ', 'wp-crm-system' ) . esc_attr( $data['mobile'] ) . '</div>';
					}
					if ( array_key_exists( 'fax', $data ) && '' != $data['fax'] ){
						echo '<div id="wpcrm_system_gdpr_table_fax">' . esc_html__( 'Fax: ', 'wp-crm-system' ) . esc_attr( $data['fax'] ) . '</div>';
					}
					if ( array_key_exists( 'email', $data ) && '' != $data['email'] ){
						echo '<div id="wpcrm_system_gdpr_table_email">' . esc_html__( 'Email: ', 'wp-crm-system' ) . esc_attr( $data['email'] ) . '</div>';
					}
					if ( array_key_exists( 'url', $data ) && '' != $data['url'] ){
						echo '<div id="wpcrm_system_gdpr_table_url">' . esc_html__( 'Website: ', 'wp-crm-system' ) . esc_attr( $data['url'] ) . '</div>';
					}
					?>
				</td>
			</tr>
			...
			<tr id="contact_additional">
				...
				<td class="wpcrm_system_gdpr_data">
					...
					if ( array_key_exists( 'information', $data ) ){
						echo '<div id="wpcrm_system_gdpr_table_information">' . esc_attr( $data['information'] ) . '</div>';
					}
					?>
				</td>
			</tr>
			...
			<tr id="contact_comments">
				...
				<td class="wpcrm_system_gdpr_data">
					<?php foreach ( $data['comments'] as $comment ){ ?>
						<div class="wpcrm_system_gdpr_comment" id="wpcrm_system_gdpr_comment_<?php echo esc_attr( $comment->ID ); ?>">
							...
							<div class="wpcrm_system_gdpr_comment_date">
								<?php echo esc_attr( $comment->comment_date ); ?>
							</div>
						</div>
					<?php } ?>
				</td>
			...
			if ( $custom_fields && is_array( $custom_fields ) ):
				foreach ( $custom_fields as $field_name => $value ){
					...
					<tr id="<?php echo 'wpcrm_system_gdpr_' . esc_attr( $field_id ); ?>">
						<th class="wpcrm_system_gdpr_title">
						...
						<td class="wpcrm_system_gdpr_data">
							<?php if ( is_array( $value ) ){
								foreach ( $value as $v ){
									echo '<div class="wpcrm_system_gdpr_table_' . esc_attr( $field_id ) . '">' . esc_html( $v ) . '</div>';
								}
							} else {
								echo '<div class="wpcrm_system_gdpr_table_' . esc_attr( $field_id ) . '">' . esc_html( $value ) . '</div>';
							} ?>
						</td>
					</tr>
				<?php
			...
			<tr id="contact_export">
				...
				<td class="wpcrm_system_gdpr_data">
					<form id="wpcrm_system_gdpr_export_contact" name="wpcrm_system_gdpr_export_contact" method="post" action="">
						<input type="hidden" name="wpcrm_system_gdpr_export_contact_nonce" value="<?php echo esc_attr( wp_create_nonce( 'wpcrm-system-gdpr-export-contact-nonce' ) ); ?>" />
			...
			<tr id="contact_change">
				...
				<td class="wpcrm_system_gdpr_data">
					...
					<form id="wpcrm_system_gdpr_remove_data" name="wpcrm_system_gdpr_remove_data" method="post" action="">
						<input type="hidden" name="wpcrm_system_gdpr_delete_contact_nonce" value="<?php echo esc_attr( wp_create_nonce( 'wpcrm-system-gdpr-delete-contact-nonce' ) ); ?>" />
