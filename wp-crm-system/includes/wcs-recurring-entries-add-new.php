...
<div class="wrap">
	<h2><?php esc_html_e( 'Add New Recurring Entry', 'wp-crm-system' ); ?> - <a href="admin.php?page=wpcrm-settings&tab=recurring&subtab=recurring-entries" class="button-secondary"><?php esc_html_e( 'Cancel - Go Back', 'wp-crm-system' ); ?></a></h2>
	...
	<form method="post" action="" class="wp_crm_system_recurring_entry_form">
		<p>
			...
			<?php echo wp_kses_post( wp_crm_system_get_post_type_list( $id = false, array( 'wpcrm-project', 'wpcrm-task' ), 'project_task_id' ) ); ?><br/>
		</p>
		<p>
		...
		<p>
			<label class="description"><?php esc_html_e( 'Repeat every', 'wp-crm-system' ); ?></label>
			<select name="number">
			<?php
			$max_number = apply_filters( 'wp_crm_system_max_recurring_entry', 365 );
			for( $a = 1; $a <= $max_number; $a++ ){ ?>
				<option value="<?php echo esc_attr( $a ); ?>"><?php echo esc_attr( $a ); ?></option>
			<?php } ?>
			</select>
			<select name="frequency">
				<option value="day"><?php esc_html_e( 'Day(s)', 'wp-crm-system' ); ?></option>
				<option value="week"><?php esc_html_e( 'Week(s)', 'wp-crm-system' ); ?></option>
				<option value="month"><?php esc_html_e( 'Month(s)', 'wp-crm-system' ); ?></option>
				<option value="year"><?php esc_html_e( 'Year(s)', 'wp-crm-system' ); ?></option>
			</select>
		</p>
